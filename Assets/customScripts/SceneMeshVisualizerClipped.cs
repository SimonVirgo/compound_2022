...
public class SceneMeshVisualizerClipped : MonoBehaviour
{
    [Tooltip("Restrict Mesh to the  axis-aligned bounding box of the objects mesh. (bounding box must be positive in all directions!) ")]
    public GameObject clipOutsideShape ;

    [Tooltip("Distort mesh to fit camera FOV")]
    public bool adaptToCamera = false;
	...
	private bool mirroredScene = false;
    ...
    void Update()
    {
        ...
        //Simon: rotate according to kinect rotation. does not work! rotate mesh instead?
        // Quaternion rotation = Quaternion.Euler(45.0f, transform.rotation.y, transform.rotation.z); //set a transformation from the kinect rotation
        // Matrix4x4 rot = Matrix4x4.Rotate(rotation); //create a rotation matrix
        // kinectToWorld *= rot; // apply rotation matrix
        
        //Simon: update kinect to world matrix based on gameObject transform
        kinectToWorld.SetTRS(gameObject.transform.position, gameObject.transform.rotation, gameObject.transform.localScale);
       // Debug.Log(kinectToWorld.rotation); //simon
				...
				(!updateWhenNoPlayers || !manager.IsUserDetected())) 
			{
				...
				for (int y = 0; y < depthHeight; y += sampleSize)
				{
					...
					for (int x = 0; x < depthWidth; x += sampleSize)
					{
                        ...
                        //vSpacePos = kinectToWorld.MultiplyPoint3x4(vSpacePos); //Simon conversion to world space is done implicitly at some point, probably because a ref of vspacePos is passed
                            ...
                            !float.IsInfinity(vSpacePos.x) && !float.IsInfinity(vSpacePos.y) && !float.IsInfinity(vSpacePos.z))
                        {
                            ...
                            //simon: disable foreground camera
                            ...
                            //mirror check original position**
                            if (!mirroredScene)
                            {
                            ...
                            vertices[vIndex] = vSpacePos; //- sceneMeshPos;  //simon This might be why the mesh does not move when we 
...
//				}
    ...
    public static bool IsInside(Collider c, Vector3 point) //function to check if point is IsInside a collider
    {
    ...
    private bool IsSceneSampleValid(int x, int y, ref Vector3 vSpacePos)
	{
        ...
        float realx = vSpacePos.x;

      if (!mirroredScene)
        {
            realx = vSpacePos.x * -1.0f;
        }
		   ...
		   (maxLeftRight < 0f || (vSpacePos.x >= -maxLeftRight && vSpacePos.x <= maxLeftRight)))
		{
            ...
            //if (clipOutsideShape == null || clipOutsideShape.GetComponent<Renderer>().bounds.Contains(kinectToWorld.MultiplyPoint3x4(vSpacePos)))// checks if a clipping box is defined, If yes, only vertices inside the box are considered.
            if (clipOutsideShape == null || IsInside( clipOutsideShape.GetComponent<Collider>(),kinectToWorld.MultiplyPoint3x4(new Vector3(realx, vSpacePos.y,vSpacePos.z)))) // was: vSpacePos without conversion
            //if (clipOutsideShape == null || clipOutsideShape.GetComponent<Mesh>().bounds.Contains(kinectToWorld.MultiplyPoint3x4(vSpacePos))) //
